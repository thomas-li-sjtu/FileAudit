<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="Pid_Killed" align="center" width="95">
        <template slot-scope="{row}">
          <span v-if="row.pid_killed" class="link-type" @click="handleFetchPv(row.pid_killed)">{{ row.pid_killed }}</span>
          <span v-else>0</span>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      getList("kill", this.listQuery).then(response => {
    ...
    handleDelete(row, index) {
      ...
      deleteRecord("kill_delete", this.deleteQuery).then(response => {
