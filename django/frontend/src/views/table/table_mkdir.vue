<template>
  <div class="app-container">
    ...
    >
      <el-table-column label="ID" prop="id" sortable="custom" align="center" width="75" :class-name="getSortClass('id')">
      ...
      <el-table-column label="DirPath" min-width="150px">
        <template slot-scope="{row}">
          <span class="link-type" @click="handleUpdate(row)">{{ row.dirpath }}</span>
      ...
      <el-table-column label="Command" align="center" width="100">
        <template slot-scope="{row}">
...
import { getList, deleteRecord } from '@/api/table'
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      getList("mkdir", this.listQuery).then(response => {
    ...
    handleDelete(row, index) {
      ...
      deleteRecord("mkdir_delete", this.deleteQuery).then(response => {
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['logtime', 'filepath', 'username', 'command', 'result']
        const filterVal = ['logtime', 'filepath', 'username', 'command', 'result']
